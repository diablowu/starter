...
/**
 * {@link ProjectGenerationConfiguration} for generation of projects that use the Kotlin language.
 *
...
class KotlinProjectGenerationConfiguration {
  ...
  @Bean
  KotlinVersionResolver kotlinVersionResolver(
      DependencyManagementVersionResolver versionResolver, InitializrMetadata metadata) {
    return new ManagedDependenciesKotlinVersionResolver(
        versionResolver,
        (description) ->
  ...
  static class KotlinCoroutinesCustomizerConfiguration {
    ...
    KotlinCoroutinesCustomizerConfiguration(
