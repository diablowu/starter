...
/**
 * {@link ProjectGenerationConfiguration} for generation of projects that depend on Spring Native.
 *
...
class SpringNativeProjectGenerationConfiguration {
  ...
  @Bean
  SpringNativeHelpDocumentCustomizer springNativeHelpDocumentCustomizer(
      InitializrMetadata metadata, ProjectDescription description, Build build) {
    return new SpringNativeHelpDocumentCustomizer(
        metadata,
        description,
        build,
  ...
  @Bean
  SpringNativeRepositoryBuildCustomizer springNativeRepositoryBuildCustomizer(
    ...
    return () -> {
      Map<String, String> resolve =
          versionResolver.resolve(
              "org.springframework.boot", "spring-boot-dependencies", springBootVersion.toString());
  ...
  private static Dependency resolveSpringNativeDependency(
  ...
  private static MavenRepository determineSpringNativeMavenRepository(
      InitializrMetadata metadata, Version platformVersion) {
    String repositoryId = resolveSpringNativeDependency(metadata, platformVersion).getRepository();
    if (repositoryId != null) {
      Repository repository =
          metadata.getConfiguration().getEnv().getRepositories().get(repositoryId);
