...
/**
 * {@link BuildCustomizer} for projects containing Spring Cloud Contract Verifier built with Gradle.
 *
...
class SpringCloudContractGradleBuildCustomizer implements BuildCustomizer<GradleBuild> {
  ...
  private static final Log logger =
      LogFactory.getLog(SpringCloudContractGradleBuildCustomizer.class);

  private static final VersionRange SPRING_CLOUD_CONTRACT_3_0_OR_LATER =
      VersionParser.DEFAULT.parseRange("3.0.0.M4");

  private static final MavenRepository SPRING_MILESTONES =
      MavenRepository.withIdAndUrl("spring-milestones", "https://repo.spring.io/milestone")
          .name("Spring Milestones")
          .build();

  private static final MavenRepository SPRING_SNAPSHOTS =
      MavenRepository.withIdAndUrl("spring-snapshots", "https://repo.spring.io/snapshot")
  ...
  SpringCloudContractGradleBuildCustomizer(
      ProjectDescription description, SpringCloudProjectVersionResolver projectsVersionResolver) {
  ...
  public void customize(GradleBuild build) {
    ...
    String sccPluginVersion =
        this.projectsVersionResolver.resolveVersion(
            platformVersion, "org.springframework.cloud:spring-cloud-contract-verifier");
            ...
            (task) -> {
              ...
              if (build.dependencies().has("webflux")) {
                ...
                build
                    .dependencies()
                    .add(
                        "rest-assured-spring-web-test-client",
